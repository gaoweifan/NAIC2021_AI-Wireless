...
def Encoder(x, feedback_bits, trainable=True):
    ...
    with tf.compat.v1.variable_scope('Encoder'):
        ...
        representation = layers.LayerNormalization(epsilon=LAYER_NORM_EPS,trainable=trainable)(encoded_patches)
        representation = layers.Flatten()(representation)
        representation = layers.Dropout(0.2)(representation)
        # Add MLP.
        features = mlp(representation, hidden_units=mlp_head_units, dropout_rate=0.2,trainable=trainable)
        # Classify outputs.
        x = layers.Dense(units=int(feedback_bits // B), activation='sigmoid',trainable=trainable)(features)
...
def Decoder(x,feedback_bits, trainable=True):
    ...
    x = layers.Dense(num_patches*projection_dim, trainable=trainable)(x)
    x = layers.BatchNormalization(trainable=trainable)(x)
    x = layers.Activation('sigmoid')(x)

    inputs = layers.Reshape((num_patches, projection_dim))(x)
    x = layers.Dense(DEC_PROJECTION_DIM,trainable=trainable)(inputs)
    ...
    x = layers.LayerNormalization(epsilon=LAYER_NORM_EPS, trainable=trainable)(x)
    x = layers.Flatten()(x)

    x = layers.Dense(512, trainable=trainable)(x)
    x = layers.BatchNormalization(trainable=trainable)(x)
    x = layers.Activation('sigmoid')(x)
    
    pre_final = layers.Dense(units=126 * 128 * 2, activation="sigmoid", trainable=trainable)(x)
    decoder_output = layers.Reshape((126, 128, 2))(pre_final)
