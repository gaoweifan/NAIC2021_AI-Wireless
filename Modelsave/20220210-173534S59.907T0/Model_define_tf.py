...
def Encoder(x, feedback_bits, trainable=True):
    ...
    with tf.compat.v1.variable_scope('Encoder'):
        x = layers.Conv2D(64, 15, padding='same', trainable=False)(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv2D(16, 15, padding='same', trainable=False)(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv2D(2, 15, padding='same', trainable=False)(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv2D(32, 15, padding='same', trainable=False)(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv2D(16, 15, padding='same', trainable=trainable)(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv2D(2, 7, padding='same', trainable=False)(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Flatten()(x)
        x = layers.Dense(units=int(feedback_bits // B), trainable=False)(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
