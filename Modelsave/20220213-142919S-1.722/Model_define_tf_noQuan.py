...
def Encoder(x, feedback_bits, trainable=True):
    ...
    with tf.compat.v1.variable_scope('Encoder'):
        ...
        representation = layers.LayerNormalization(epsilon=LAYER_NORM_EPS,trainable=trainable,name="enc_ln_1")(encoded_patches)
        representation = layers.Flatten()(representation)
        representation = layers.Dropout(0.2)(representation)
        # Add MLP.
        features = mlp(representation, hidden_units=mlp_head_units, dropout_rate=0.2,trainable=trainable)
        # Classify outputs.
        x = layers.Dense(units=int(feedback_bits // B), activation='sigmoid',trainable=trainable,name="enc_dense_1")(features)
        encoder_output = x
...
def Decoder(x,feedback_bits, trainable=True):
    ...
    pre_final = layers.Dense(units=126 * 128 * 2, activation="sigmoid", trainable=trainable,name="dec_dense_5")(x)
