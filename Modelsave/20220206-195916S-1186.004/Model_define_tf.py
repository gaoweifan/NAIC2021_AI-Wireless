...
def Encoder(x, feedback_bits, trainable=True):
    ...
    with tf.compat.v1.variable_scope('Encoder'):
        ...
        x = layers.Reshape((126, 2, 64, 2))(x)
        x = layers.Permute((2,1,3,4))(x)
        # x = tf.expand_dims(x,1)
        x = layers.ConvLSTM2D(2, kernel_size=(7, 7), activation='sigmoid', padding='same',return_sequences=True, trainable=trainable)(x)
        x = layers.ConvLSTM2D(2, kernel_size=(7, 7), activation='sigmoid', padding='same',return_sequences=False, trainable=trainable)(x)
...
def Decoder(x,feedback_bits, trainable=True):
    ...
    x = layers.RepeatVector(2)(x)
    x = layers.Dense(16128, activation='sigmoid', trainable=trainable)(x)
    x = layers.Reshape((2, 126, 64, 2))(x)
