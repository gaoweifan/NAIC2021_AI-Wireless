...
def Encoder(x, feedback_bits, trainable=True):
    ...
    with tf.compat.v1.variable_scope('Encoder'):
        x = layers.Reshape((126, 128, 2, 1))(x)

        x = layers.Conv3D(32, (7,7,2), padding='same', trainable=trainable,name="enc_conv3d_1_32")(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv3D(8, (7,7,2), padding='same', trainable=trainable,name="enc_conv3d_2_8")(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv3D(1, (7,7,2), padding='same', trainable=trainable,name="enc_conv3d_3_1")(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv3D(16, (7,7,2), padding='same', trainable=trainable,name="enc_conv3d_4_16")(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv3D(8, (7,7,2), padding='same', trainable=trainable,name="enc_conv3d_5_8")(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)

        x = layers.Conv3D(1, (7,7,2), padding='same', trainable=trainable,name="enc_conv3d_6_1")(x)
        x = layers.BatchNormalization(trainable=trainable)(x)
        x = layers.LeakyReLU(alpha=0.1)(x)
        # x = layers.Activation('relu')(x)
...
def Decoder(x,feedback_bits, trainable=True):
    ...
    x = layers.Conv3D(16, (7,7,2), padding='same', trainable=trainable,name="dec_conv3d_1_16")(x)
    x = layers.BatchNormalization(trainable=trainable)(x)
    x = layers.LeakyReLU(alpha=0.1)(x)

    x = layers.Conv3D(8, (7,7,2), padding='same', trainable=trainable,name="dec_conv3d_2_8")(x)
    x = layers.BatchNormalization(trainable=trainable)(x)
    x = layers.LeakyReLU(alpha=0.1)(x)

    x = layers.Conv3D(4, (7,7,2), padding='same', trainable=trainable,name="dec_conv3d_3_4")(x)
    x = layers.BatchNormalization(trainable=trainable)(x)
    x = layers.LeakyReLU(alpha=0.1)(x)

    x = layers.Conv3D(8, (7,7,2), padding='same', trainable=trainable,name="dec_conv3d_4_8")(x)
    x = layers.BatchNormalization(trainable=trainable)(x)
    x = layers.LeakyReLU(alpha=0.1)(x)

    x = layers.Conv3D(4, (7,7,2), padding='same', trainable=trainable,name="dec_conv3d_5_4")(x)
    x = layers.BatchNormalization(trainable=trainable)(x)
    x = layers.LeakyReLU(alpha=0.1)(x)

    x = layers.Conv3D(1, (7,7,2), padding='same', trainable=trainable,name="dec_conv3d_6_1")(x)
    x = layers.BatchNormalization(trainable=trainable)(x)
    x = layers.Activation('sigmoid')(x)
